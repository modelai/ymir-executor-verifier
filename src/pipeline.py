    ...
    def run(self):
        ...
        for idx, task in enumerate(self.cfg.tasks):
            ...
            if idx == 0 and task == 'training':
                v.verify_task(docker_image_name=self.cfg.docker_image, task=task)
                new_weights_dir = osp.join(self.cfg.work_dir, self.task_id, task, 'models')

                # copy weights file to other path to mount in docker
                self.host_out_dir = self.cfg.out_dir
                host_weights_dir = self.get_host_path(self.cfg.env_config.output.models_dir)
